...
produced. It was updated in April 2026 by Prof Perry to fix issues caused by updates to the Torch package.
...
transform <- function(x) {
  x %>%
    transform_to_tensor()  # returns a 1 x 28 x 28 tensor scaled to [0, 1]
}
...
```{r}
dl_mnist <- dataloader(train_ds, batch_size = 256)
b_mnist <- dataloader_make_iter(dl_mnist) %>% dataloader_next()
b_mnist[[1]]$shape
b_mnist[[2]]$shape
```
...
```{r}
setwd("R:/Courses/STAT 140B/Labs") # you will need to update this! #
img_dir <- "book_images"
...
model <- nn_module(
  ...
  forward = function(x) {
    c(output, c(hn, cn)) %<-% (x %>%
      self$embedding() %>%
      self$lstm())
    output[, output$size(2), ] %>%  # get the last output in the sequence
      self$dense() %>%
      torch_flatten(start_dim = 1)
  }
)
